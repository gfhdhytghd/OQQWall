...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_friend_recall(self, data):
        ...
        if user_id and message_id:
            try:
                conn = sqlite3.connect('OQQWall.db')
                cursor = conn.cursor()
        ...
        if message_type == 'private' and post_type != 'message_sent' and user_id and timestamp is not None:
            ...
            try:
                ...
                if row:
                    ...
                    cursor.execute('INSERT INTO sender (senderid, receiver, ACgroup, rawmsg) VALUES (?, ?, ?, ?)',
                    ...
                    cursor.execute('INSERT INTO preprocess (tag, senderid, receiver, ACgroup) VALUES (?, ?, ?, ?)',
                                   (new_tag, user_id, self_id, ACgroup))

                    # Commit changes
                    conn.commit()

                    # Call the preprocess.sh script with the new tag
                    preprocess_script_path = './preprocess.sh'
                    try:
