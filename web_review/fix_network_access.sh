...
# 使用后台运行
nohup python3 web_review.py --host 0.0.0.0 --port 8090 > web_review.log 2>&1 &
SERVICE_PID=$!
...
if ps -p $SERVICE_PID > /dev/null 2>&1; then
    ...
    # 测试服务是否响应
    echo "🔍 测试服务响应..."
    if curl -s http://localhost:8090/api/stats > /dev/null 2>&1; then
        echo "✅ 服务响应正常"
    else
    ...
    echo ""
    echo "💡 现在可以通过以下方式访问："
    echo "   - 在浏览器中访问: http://$LOCAL_IP:8090"
    echo "   - 或访问: http://localhost:8090"
    echo ""
